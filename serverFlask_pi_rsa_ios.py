...
app = Flask(__name__, template_folder = '/home/pi/Longmax/html', static_folder = '/home/pi/Longmax/html', static_url_path='')
app.debug = True
...
def rsaEncodeDB(remoteDeviceID, text):
    sql1 = "SELECT * FROM PeerInfo where ChargerID = '%s'" %(remoteDeviceID)
...
def decrypt_with_private_key_for_device_id(_cipher_text, deviceId):
    ...
    :return:
    """
    sql1 = "SELECT ChargerID, PublicPEM, PrivatePEM FROM IOSPeerInfo WHERE ChargerID = %s"
...
#### float conversion is for control card (no bit shifting issue)
def floatConversion(ls):
    string=''
    ...
    print("Converted Float to String:", string)
    exponent = 0
    for i in range(1,9):
        if string[i] == '1':
            exponent += 2**(8-i)
...
def CMDConversion(Mode, number): #convert the formating of 32-bit sending data to be the one TI can decode
    ...
    prefix = [17,34,51,68,85,102,119,136,153,18] #prefix to be converted in to hex number, which can be used as the flag for each mode through SPI communication
...
### SPI function for control card:
def sendSPIControlCard(Mode, number):
    Modes = ['M','S','PF','PP','PN','QP','QN','OnOff','Current', 'kWh']
    ...
    spi.close()
    spi.open(bus, device)
    spi.max_speed_hz = 976000 #3.9 MHz
    spi.mode = 0b00
    while (cnt<5):
        global flt
        global flt1
        ...
        line1 = str(t1)
        data = [t1]
        b_list = CMDConversion(Mode, number)
        b_list_temp = b_list.copy()
        #b = spi.readbytes(64)
        print('Oringinal Data sent',b_list)
        b = spi.xfer2(b_list)
        print('Received Raw Data after xfer2',b_list_temp)
        print('b',b)
        spi.close()
        # print(len(b))
        # print("B", b)
        ...
        for ind in range(8):
            a1 = b[4*ind:(4*ind+4)]
            print("Raw Data Received for [", ind, "]:", a1)
            flt = floatConversion(a1)
            print("Converted Flot for [", ind, "]:", flt)
            data.append(flt)
        print("----------- Data Received at Cnt", cnt, "is:", data)

        if int(data[4]) == param:
            print("##########################")
            ack = data
            print("The Acknowledgement: ", ack)
            return ack
            break
...
def send_ACK():
    ...
    if request.method == 'POST':
        ...
        db.commit()
        print("####  success  to  MariaDB  ####", LocalDeviceID)
    
        res = LocalDeviceID + ", Insert ACK successfully"
...
def Log_data():
    ...
    if request.method == 'POST':
        ...
        line1 = str(time)+', '+str(data_1)+', '+str(data_2)+', '+str(data_3)+', '+str(data_4)+', '+str(data_5)+', '+str(data_6)+', '+str(data_7)+', '+str(data_8)
        line = line1 +', '+'\n'
...
def Forward_CMD():
    ...
    if request.method == 'POST':
        data = request.get_data()
        data_str=data.decode('UTF-8')
        data_str=data_str.replace('Time=','{"Time": ').replace('&deviceID=',',"deviceID": "').replace('&Sender=','","Sender": "').replace('&Command=','","Command": "').replace('&Value=','","Value": ')
        data_str=data_str+'}'
        data_json=eval(data_str)
        ...
        if resp_sender == resp_deviceID:
            ...
            sql = "SELECT * FROM PeerInfo where ChargerID = '%s'" %(resp_sender)
            db = pymysql.connect('localhost','nie','nie','longmax')
            cursor = db.cursor()
            cursor.execute(sql)
            result=cursor.fetchall()
            ...
            if len(result)<1:
                ...
                print("*****CMD Recived from Web App: Time:", resp_time ,"Commmand:", resp_cmd , "Value Entered:", resp_value)
                ...
                ack_json = {"Time": t1, "Success": Success, "Value": Value, "Parameter": Parameter, "CMD_Time": resp_time, "Sender": LocalDeviceID}
                ...
                flask_url = 'http://picharger02.www.mplabcharger.org:9063/CMD/ReceiveCMD'
                response = requests.post(flask_url, data=json.dumps(cmd_json), verify=False)


                res = localDeviceID+"Successfully update the ACK to the database"
            ...
            # 1209
            # cipher_text = rsaEncode(resp_deviceID, str(cmd_json)) ### use the receiver's public key to encrypt the original message
            ...
            # signature = createSignature(resp_sender, str(cmd_json)) ### use the sender's private key to create signature
            ...
            # flask_url = 'http://'+resp_deviceID+'.www.mplabcharger.org:9063/CMD/ReceiveCMD'
            # print(flask_url)
...
def Receive_CMD():
    ...
    if request.method == 'POST':
        ...
        # Check if the sender is trusted or not.
        sql = "SELECT * FROM PeerInfo where ChargerID = '%s'" %(sender)
        db = pymysql.connect('localhost','nie','nie','longmax')
        cursor = db.cursor()
        cursor.execute(sql)
        result=cursor.fetchall()
        ...
        if len(result)<1:
            ...
            res = "LocalDeviceID, Successfully the data to the database"
        res = "Received CMD"
        rsp = Response(json.dumps(res), status=200, content_type="application/json")
        return rsp
...
def Send_to_Peer():
    ...
    if request.method == 'POST':
        data = request.get_data()
        data_str=data.decode('UTF-8')
        data_str=data_str.replace('LastName=','{"LastName": "').replace('&FirstName=','","FirstName": "').replace('&Username=','","Username": "').replace('&Password=','","Password": "').replace('&Email=','","Email": "').replace('%40','@').replace('&Role=','","Role": "')
        data_str=data_str+'"}'
        print(data_str)
        data_json=eval(data_str)
        ...
        sql1 = "SELECT * FROM UserPWD WHERE Username = %s" 
        val = (str(Username))
        db1 = pymysql.connect('localhost','nie','nie','longmax')
        cursor1 = db1.cursor()
        ...
        if length>0:
            ...
            sql0 = "INSERT INTO UserPWD (LastName, FirstName, Username, Password, Email, Role) VALUES (%s, %s, %s, password(%s), %s, %s)" 
            ...
            db = pymysql.connect('localhost','nie','nie','longmax')
            cursor = db.cursor()
            cursor.execute("SELECT ChargerID from PeerInfo")
            ...
            for index in range(len(peer_DeviceID)):
                if peer_DeviceID[index] != LocalDeviceID:
                    flask_url = 'http://'+peer_DeviceID[index]+'.www.mplabcharger.org:9063/UserInfo/ReceivefromPeer'
                    print(flask_url)
                    senderInfo = {"Sender":LocalDeviceID}
                    data_json.update(senderInfo)
                    print("The Json send to the peers: ", data_json)
...
def Receive_from_Peer():
    ...
    if request.method == 'POST':
        ...
        if len(result)<1:
            ...
            sql = "INSERT INTO UserPWD (LastName, FirstName, Username, Password, Email, Role) VALUES (%s, %s, %s, password(%s), %s, %s)" 
            val = (str(LastName), str(FirstName), str(Username), str(Password), str(Email), str(Role))
            print(sql)
            db= pymysql.connect('localhost','nie','nie','longmax')
            cursor = db.cursor()
            cursor.execute(sql, val)
            db.commit()
            print("Successfully Insert a New UserInfo: ", sql)
...
def Check_User_Authority():
    ...
    if request.method == 'POST':
        ...
        print('Owner:', Owner, 'Group:', Group, "Owner Rights:", OwnerRights, "GroupRights:", GroupRights, "EveryRights:", EveryRights)
...
def Login_Check():
    ...
    if request.method == 'POST':
        ...
        sql = "SELECT * FROM UserPWD where Username=%s and Password=password(%s)"
        ...
        sql1 = "SELECT * FROM VehicleStatus ORDER BY time DESC LIMIT 1";
        #sql = "SELECT * FROM DeviceInfo where ChargerID = '%s'" %(deviceID)
        db1 = pymysql.connect('localhost','nie','nie','longmax')
        cursor1 = db1.cursor()
        cursor1.execute(sql1)
        ...
        Voltage=result1[0][1]
        Current=result1[0][2]
        TimeToFull=result1[0][3]
        ChargingDischarging=result1[0][4]
        EVStatus=result1[0][5]
        print("EVStatus:", EVStatus)

        #0: If the username and password is not matched, respond 0
        #1: If the username and password is correct in the database and charger is not occupied, respond 1
        #2: If the username and password is correct in the database and charger is occupied by other users, respond2
        #3: If the username and password is correct in the database and charger is not occupied,
        #the login user is not an admin and the EV is not plugged
        
        #Php will redirect to different webpage according to the response (2/1/0)
        if length>0:
            if EVStatus == 1 and success == 0:
                res = "3"
            else:
...
@app.route('/UserInfo/Logout', methods=['Post','GET'])
def Logout():
    res = None
    if request.method == 'POST':
        #Get the data from the PHP (array)
        ...
        print(data)
        data_str=data.decode('UTF-8')
        data_str=data_str.replace('Username=','{"Username": "').replace('&deviceID=','","deviceID": "')
        data_str=data_str+'"}'
        data_json=eval(data_str)
        Username = data_json['Username']
        deviceID = data_json['deviceID']
...
def NetworkOfTrust_SenderLocal():
    ...
    if request.method == 'POST':
        ...
        Username=data_json["Username"] 
        json_data = {"Sender": LocalDeviceID, "Username": Username, "Method": "DeviceID"} #**********
        flask_url = 'http://'+deviceID+'.www.mplabcharger.org:9063/NetworkOfTrust/Receiver'
        print(flask_url)
...
def NetworkOfTrust_SenderLocalIP():
    ...
    if request.method == 'POST':
        ...
        Username=data_json["Username"] 
        json_data = {"Sender": LocalDeviceID, "Username": Username, "Method": "DeviceIP"} #**********
...
def NetworkOfTrust_SenderLocalRemove():
    ...
    if request.method == 'POST':
        ...
        if "Remove" in data_str:
            ...
            db = pymysql.connect('localhost','nie','nie','longmax')
            cursor = db.cursor()
            cursor.execute("SELECT ChargerID,PublicPEM from PeerInfo")
            fetch_data = cursor.fetchall()
            print(fetch_data)
            for ChargerID in fetch_data:
                peer_DeviceID.append(ChargerID[0])
            ...
            for index in range(len(peer_DeviceID)):
                if peer_DeviceID[index] != LocalDeviceID:
                    ...
                    flask_url = 'http://'+peer_DeviceID[index]+".www.mplabcharger.org:9063/NetworkOfTrust/ReceiverRemove"
                    sql1 = "DELETE FROM PeerInfo WHERE ChargerID = '%s'"%(peer_DeviceID[index])
                    db1 = pymysql.connect('localhost','nie','nie','longmax')
                    cursor1 = db1.cursor()
                    cursor1.execute(sql1)
                    db1.commit()
...
def NetworkOfTrust_ReceiverRemove():
    ...
    if request.method == 'POST':
        ...
        if CMD == "Remove":
            ...
            sql1 = "DELETE FROM PeerInfo WHERE ChargerID = '%s'"%(Sender)
            db1 = pymysql.connect('localhost','nie','nie','longmax')
            cursor1 = db1.cursor()
            cursor1.execute(sql1)
            db1.commit()
    ...
    if request.method == 'POST':
        ...
        if Method == "DeviceIP":
            ...
            if len(result)>0:
                ...
                def ExitApplication():
                    global result_res
                    MsgBox = tk.messagebox.askquestion ('New Connection Request','Do you want to accept the connection from '+deviceID+' send by user: '+Username+'.' ,icon = 'warning')
                    if MsgBox == 'yes':
                        sql = "SELECT * FROM PeerInfo where ChargerID = '%s'" %(LocalDeviceID)
                        db1 = pymysql.connect('localhost','nie','nie','longmax')
                        cursor1 = db1.cursor()
                        cursor1.execute(sql)
                        result=cursor1.fetchall()
                        ...
                        flask_url = 'http://'+deviceID+'.www.mplabcharger.org:9063/NetworkOfTrust/Sender'
                        print(flask_url)
                        response = requests.post(flask_url, data=json.dumps(json_data), verify=False)
                        print('Response from the peer:', response.text)
                        root.destroy()
...
def NetworkOfTrust_Sender():
    ...
    if request.method == 'POST':
        ...
        gateDeviceURL = 'http://'+gateDeviceID+'.www.mplabcharger.org'
        sql = "INSERT INTO PeerInfo (ChargerID, ChargerURL, PublicPEM) VALUES ('%s', '%s', '%s')" %(gateDeviceID, gateDeviceURL, gatePublicKey)
        db = pymysql.connect('localhost','nie','nie','longmax')
        cursor = db.cursor()
        cursor.execute(sql)
        db.commit()

        sql1 = "SELECT * FROM PeerInfo where ChargerID = '%s'" %(LocalDeviceID)
        db1 = pymysql.connect('localhost','nie','nie','longmax')
        cursor1 = db1.cursor()
        cursor1.execute(sql1)
        result=cursor1.fetchall()
        ...
        flask_url = 'http://'+gateDeviceID+'.www.mplabcharger.org:9063/NetworkOfTrust/ReceiveNewPublic'
        print(flask_url)
        response = requests.post(flask_url, data=json.dumps(json_data), verify=False)
        print('Response from the peer:', response.text)
        ...
        for i in range(len(res_all)-1):
            ...
            Key = tempIDandKey[1]
            print(ID,Key)
            URL = 'http://'+ID+'.www.mplabcharger.org'
            sql2 = "INSERT INTO PeerInfo (ChargerID, ChargerURL, PublicPEM) VALUES ('%s', '%s', '%s')" %(ID, URL, Key)
            db2 = pymysql.connect('localhost','nie','nie','longmax')
            cursor2 = db2.cursor()
    ...
    if request.method == 'POST':
        ...
        for index in range(len(peer_DeviceID)):
            if peer_DeviceID[index] != LocalDeviceID:
                ...
                forward_data_json={"Sender": LocalDeviceID, "NewDeviceID": newDeviceID, "PublicKey": encryptedNewPublicKey}

                returnAllOtherPeerInfo += peer_DeviceID[index]+'innerSeparate'+peer_key[index]
                returnAllOtherPeerInfo += 'outerSeparate'

                ####
                flask_url = 'http://'+peer_DeviceID[index]+'.www.mplabcharger.org:9063/NetworkOfTrust/PeerReceiveNewPublic'
                print(flask_url)
                response = requests.post(flask_url, data=json.dumps(forward_data_json), verify=False)
                print('Response from the peer', peer_DeviceID[index], ':', response.text)

        print("------------------- Successfully Send the Information of New Device to Everyone ----------------------- ")

        ## Insert the public key of the new device into the DB of the gateway device
        newDeviceURL = 'http://'+newDeviceID+'.www.mplabcharger.org'
        sql = "INSERT INTO PeerInfo (ChargerID, ChargerURL, PublicPEM) VALUES ('%s', '%s', '%s')" %(newDeviceID, newDeviceURL, publicKeyDecode)
        db = pymysql.connect('localhost','nie','nie','longmax')
        cursor = db.cursor()
        cursor.execute(sql)
        db.commit()
        print("------------------- Successfully Store the Information of New Device in the Gateway DB ----------------------- ")

        print(returnAllOtherPeerInfo)

        return jsonify({'res':returnAllOtherPeerInfo})
...
def NetworkOfTrust_PeerReceiveNewPublic():
    ...
    if request.method == 'POST':
        ...
        if len(result)<1:
            ...
            ## Insert the public key of the new device into the DB of the gateway device
            newDeviceURL = 'http://'+newDeviceID+'.www.mplabcharger.org'
            sql1 = "INSERT INTO PeerInfo (ChargerID, ChargerURL, PublicPEM) VALUES ('%s', '%s', '%s')" %(newDeviceID, newDeviceURL, publicKeyDecode)
            db1 = pymysql.connect('localhost','nie','nie','longmax')
            cursor1 = db1.cursor()
            cursor1.execute(sql1)
            db1.commit()
            print("------------------- Successfully Store the Information of New Device in the Peer DB ----------------------- ")
    ...
    if request.method == 'POST':
        ...
        #find out all the peers for this charger:
            ...
            for i in range(0, len(result)):
                res += "<tr><td>"+str(result[i][0])+"</td><td>"+str(result[i][1])+"</td><td>"+str(result[i][3])+"</td><td>"+str(result[i][4])+"</td></tr>"
...
def Forget_Password():
    ...
    if request.method == 'POST':
        ...
        if length>0:
            ...
            sql2 = "SELECT * FROM UserPWD WHERE Username = %s"
            val2 = (str(OwnerName))
            db2 = pymysql.connect('localhost','nie','nie','longmax')
            cursor2 = db2.cursor()
            ...
            Email = result[0][5]
            res = "<tr><th> Owner Name: </th><td>"+str(OwnerName)+"</td></tr><tr><th> Email Address: </th><td>"+str(Email)+"</td></tr>"
...
@app.route('/Service/BuyPower', methods=['Post','GET']) # route connected with serviceBuy.html and conncet_buy.php
def Buy_Power():
    ...
    if request.method == 'POST':
        ...
        data_str=data.decode('UTF-8')
        print(data_str)
        data_str=data_str.replace('method=','{"method": "').replace('&amount=','","amount": "').replace('&time=','","time": "')
        data_str=data_str+'"}'
        print(data_str)
        data_json=eval(data_str)
        method=data_json["method"]
        ...
        if method == "1": #if the user choose to charge full tank
            ...
            Mode = "S"
            valueS = 2
            sendSPIControlCard(Mode, valueS)
            # Let TI know the user want to charge full tank (might need to get from BMS SOC?)
            sql0 = "SELECT * FROM CMD"+LocalDeviceID+" where Time ='%s'" %(resp_time)
            db0= pymysql.connect('localhost','nie','nie','longmax')
            cursor0 = db0.cursor()
            cursor0.execute(sql0)
            result0=cursor0.fetchall()
            if len(result0)<1:
                # Write to MariaDB
                sql1 = "INSERT INTO `CMDpicharger01` (Time, Command, Value) VALUES (%s, %s, %s)"
                data1 = (str(resp_time), str(Mode), str(valueS))
                db1= pymysql.connect('localhost','nie','nie','longmax')
                cursor1 = db1.cursor()
                cursor1.execute(sql1,data1)
                db1.commit()
                print("####  "+LocalDeviceID+"Charger 01 successfully insert into  MariaDB  ####")

            Mode = "kWh"
            value = 65535
            sendSPIControlCard(Mode, value)
            ...
            if Voltage != number[0]:
                ...
                sendSPIControlCard(Mode, number[0])
            if Current != number[1]:
                number[1] = Current
                Mode = "Current"
                print(Mode, number[1])
                # send current value to TI
                sendSPIControlCard(Mode, number[1])
            res = "1"
            ...
            Mode = "kWh"
            value1 = int(value)
            sendSPIControlCard(Mode, value1)
...
@app.route('/Service/SellPower', methods=['Post','GET']) # route connected with serviceSell.html and conncet_buy.php
...
def Sell_Power():
    ...
    if request.method == 'POST':
        ...
        data_str=data.decode('UTF-8')
        print(data_str)
        data_str=data_str.replace('value=','{"value": "').replace('&hours=','","hours": "').replace('&time=','","time": "')
        data_str=data_str+'"}'
        print(data_str)
        data_json=eval(data_str)
        kWh=data_json["value"]
        ...
        ModeS = "S"
        valueS = -2
        acks = sendSPIControlCard(ModeS, valueS)
        # Let TI know the amount the user want to discharge
        kWh=int(kWh)
        ModekWh = "kWh"
        valuekWh = kWh
        ackkWh = sendSPIControlCard(ModekWh, valuekWh)
        ...
        if Voltage != number[0]:
            ...
            print(ModeP, number[0])
            time.sleep(1)
            # send Voltage value to TI
            ackV=sendSPIControlCard(ModeP, Voltage)
        if Current != number[1]:
            number[1] = Current
            ModeC = "Current"
            print(ModeC, number[1])
            # send current value to TI
            ackC=sendSPIControlCard("Current", Current)
        ...
        if len(result0)<1:
            # Write to MariaDB
            sql1 = "INSERT INTO `CMDpicharger01` (Time, Command, Value, ACK_Time, ACK_Success, ACK_Value, ACK_Parameter) VALUES (%s, %s, %s, %s, %s, %s, %s)"
            data1 = (str(resp_time), str(ModeS), str(valueS), str(resp_time), "1", str(valueS), "1")
            db1= pymysql.connect('localhost','nie','nie','longmax')
            cursor1 = db1.cursor()
            cursor1.execute(sql1,data1)
            db1.commit()
            ...
            sql1 = "INSERT INTO `CMDpicharger01` (Time, Command, Value, ACK_Time, ACK_Success, ACK_Value, ACK_Parameter) VALUES (%s, %s, %s, %s, %s, %s, %s)"
            data1 = (str(time.time()), str(ModekWh), str(valuekWh), str(time.time()), "1", str(valuekWh), "7")
            db1= pymysql.connect('localhost','nie','nie','longmax')
            cursor1 = db1.cursor()
            cursor1.execute(sql1,data1)
            db1.commit()
            ...
            sql1 = "INSERT INTO `CMDpicharger01` (Time, Command, Value, ACK_Time, ACK_Success, ACK_Value, ACK_Parameter) VALUES (%s, %s, %s, %s, %s, %s, %s)"
            data1 = (str(time.time()), str(ModeP), str(Voltage), str(time.time()), "1", str(Voltage), "3")
            db1= pymysql.connect('localhost','nie','nie','longmax')
            cursor1 = db1.cursor()
            cursor1.execute(sql1,data1)
            db1.commit()
            ...
            sql1 = "INSERT INTO `CMDpicharger01` (Time, Command, Value, ACK_Time, ACK_Success, ACK_Value, ACK_Parameter) VALUES (%s, %s, %s, %s, %s, %s, %s)"
            data1 = (str(time.time()), str(ModeC), str(Current), str(time.time()), "1", str(Current), "6")
            db1= pymysql.connect('localhost','nie','nie','longmax')
            cursor1 = db1.cursor()
            cursor1.execute(sql1,data1)
            db1.commit()
        ...
        res = "[1]state:"+str(ACK_State)+"[7]kWh: "+str(ackkWh[6])+"[3] EV Real-time Active Power: "+str(number[0])+"[4] EV Realtime Current"+str(number[1])
        print(res)
...
@app.route('/Service/UserSessionCheck', methods=['Post','GET']) # route to check the Login User SESSION

def User_SessionCheck():
    result = None
    if request.method == 'POST':
        #decode the array send from connect_sell.php and jsonfy the array
        data = request.get_data()
        data_str=data.decode('UTF-8')
        print(data_str)
        data_str=data_str.replace('deviceID=','{"deviceID": "').replace('&Username=','","Username": "')
        data_str=data_str+'"}'
        print(data_str)
        data_json=eval(data_str)
        deviceID=data_json["deviceID"]
        ...
        result = "Invalid request for /Service/UserSessionCheck."
        return result, 400, {'Content-Type': 'text/plain; charset=utf-8'}
...
def Show_Charger_Resp():
    ...
    if request.method == 'POST':
        ...
        res = "<tr><th>Charger/Discharge:   </th><td>"+ACK_State_Str+"</td></tr>"+"<tr><th>kWh Charged: </th><td>"+str(ACK_kWh)+"</td></tr>"
...
@app.route('/showSellEngine', methods=['Post', 'GET'])
...
def Mobile_Send_to_Peer():
    ...
    if request.method == 'POST':
        ...
        if md5String != this_md5:
            result = encry("Not Match", public_pem_)
            return result, 400, {'Content-Type': 'text/plain; charset=utf-8'}
        ...
        if length > 0:
            ...
            db = pymysql.connect('localhost','nie','nie','longmax')
            cursor = db.cursor()
            cursor.execute("SELECT ChargerID from IOSPeerInfo")
            fetch_data = cursor.fetchall()
            db.close()
            print(fetch_data)
            for ChargerID in fetch_data:
...
def get_pem_by_device_id():
    ...
    if len(res) == 0:
        ...
        insert pem
        """
        sql_inser = "insert into IOSPeerInfo (ChargerID, PublicPEM, PrivatePEM) values (%s, %s, %s)"
